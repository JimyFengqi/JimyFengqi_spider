    ...
    while True:
        ...
        for event in dev.read():
           ...
           if (event.value == 1 or event.value == 0) and event.code != 0:
               if event.value:
                   print "Key: %s Status:pressed" % (event.code)
                   write_content=key[event.code]
               else:
                   print "Key: %s Status:release" % (event.code)     
                #print "Key: %s Status: %s" % (key[event.code], "pressed" if event.value else "release")
               print(write_content)
